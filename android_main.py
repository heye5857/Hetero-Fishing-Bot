...
import cv2
import numpy as np
from vision import get_fishing_positions, check_image, grab_screen_android
from actions import press_left, press_right, stop_moving, interact, click_at, press_left_android, press_right_android, stop_moving_android, interact_android, click_at_android
from states import BotState
from android_config import CENTER_THRESHOLD, LOOP_DELAY, FISHING_REGION
import threading

def main():
    """
    Android版本钓鱼机器人主程序
    使用ADB和Python库控制Android设备
    """
    state = BotState.IDLE
    print("Android機器人已啟動。按下 Ctrl+C 可停止。")
    
    try:
        while True:
            # 只有在非 QTE 狀態下才擷取全螢幕
            if state != BotState.QTE:
                screen = grab_screen_android()
            ...
            if state == BotState.IDLE:
                ...
                if green_x is not None and cursor_x is not None:
                    ...
                    stop_moving_android()
                    full_screen = grab_screen_android()
                    ...
                    if success_detected:
                        ...
                        if success_loc:
                            ...
                            click_at_android(success_loc[0] + 50, success_loc[1] + 20)
                        time.sleep(1.5)
                        state = BotState.IDLE
                    else:
                        # 2. 檢查是否失敗重回開始畫面
